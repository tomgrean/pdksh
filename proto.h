...
int 	x_bind(const char *a1, const char *a2, int macro,
			      int list);
int		set_completion(const char *name, const char *opts);
/* eval.c */
...
int 	execute(struct op * volatile t, volatile int flags, volatile int * volatile xerrok);
...
int	set_block_mode(int fd, int blk);
