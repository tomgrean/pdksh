...
init_complete(void)
{
	tinit(&completes, APERM, 0);
}

int
set_completion(const char *name, const char *opts)
{
	char *n, *x;
	struct tbl *p;
...
#define COMP_STATE_INIT  0
#define COMP_STATE_CMD   1
#define COMP_STATE_FILE  2
...
create_argv(const char *buf, int len)
{
	...
	for (i = 8; i <= argc; i <<= 1) {
		if (i == argc) {
			e->loc->argv = aresize(e->loc->argv, sizeof(char*) * (argc<<1), ATEMP);
			break;
		}
	}
...
static int
parse_input_str(const char *buf, const char *max, int parse_last,
...
/*
 * 1. parse the command name and argument. do function callback with *func.
 * 2. in callback function, check the state machine and find if the completion
 *        is available, change the state of machine if possible.
 * 3. back in callback function's return. check if error occured.
...
x_parameter_glob(const char *buf, const char *str, int slen, char ***wordsp, int *is_commandp)
{
	...
	if (buf == str) {
		...
		ret = parse_input_str(buf, str, 0, deal_with_part);
	}
	...
	if (ret >= 0) {
		...
		case COMP_STATE_UFUNC:
			if (completion_state.cmd_info) {
				if ('S' == completion_state.cmd_info->type) {
					...
					*wordsp = (char**)XPclose(w);
					return left;
				} else if ('F' == completion_state.cmd_info->type) {
					struct tbl *f;
					...
					if (f) {
						...
						e->loc->argc = 0;
						e->loc->argv = alloc(sizeof(char*) * 8, ATEMP);
						parse_input_str(val, val + strlen(val), 1, create_argv);
						e->loc->argv[e->loc->argc] = NULL;
